...
3.1.2 使用最优化算法计算最终的权重值

val weightsWithIntercept = optimizer.optimize(data, initialWeightsWithIntercept)
  有梯度下降算法和L-BFGS两种算法来计算最终的权重值，查看梯度下降法和L-BFGS了解详细实现。 这两种算法均使用Gradient的实现类计算梯度，使用Updater的实现类更新参数。在LogisticRegressionWithSGD和LogisticRegressionWithLBFGS中，它们均使用LogisticGradient实现类计算梯度，使用SquaredL2Updater实现类更新参数。
